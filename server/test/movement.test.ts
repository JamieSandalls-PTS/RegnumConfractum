...
describe('server-authoritative movement (D-102, D-104)', () => {
  it('applies a move intent one tile per cooldown window', () => {
    ...
    const { entity } = world.spawn('tiny-test', { characterId: 'char-1', name: 'Mover', pos: { x: 1, y: 1 } });
  ...
  it('rejects walking into walls and out of bounds, but still turns to face', () => {
    const world = makeWorld();
    const { entity } = world.spawn('tiny-test', { characterId: 'char-1', name: 'Wallhugger', pos: { x: 1, y: 1 } });
  ...
  it('does not cut corners diagonally', () => {
    ...
    const { entity } = world.spawn('tiny-test', { characterId: 'char-1', name: 'Cutter', pos: { x: 3, y: 2 } });
  ...
  it('spawns at area spawn when requested tile is not walkable', () => {
    const world = makeWorld();
    const { entity } = world.spawn('tiny-test', { characterId: 'char-1', name: 'Faller', pos: { x: 4, y: 3 } });
    expect(entity.pos).toEqual({ x: 1, y: 1 });
  });

  it('latest intent wins; intents are consumed once applied', () => {
    const world = makeWorld();
    const { entity } = world.spawn('tiny-test', { characterId: 'char-1', name: 'Fickle', pos: { x: 2, y: 2 } });
    world.setMoveIntent(entity.id, 'e');
  ...
  it('emits entity_moved events only for actual moves, per area', () => {
    const world = makeWorld();
    const a = world.spawn('tiny-test', { characterId: 'char-a', name: 'Alpha', pos: { x: 1, y: 1 } }).entity;
    const b = world.spawn('tiny-test', { characterId: 'char-b', name: 'Beta', pos: { x: 2, y: 6 } }).entity;
